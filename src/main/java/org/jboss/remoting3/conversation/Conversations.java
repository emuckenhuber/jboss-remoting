...
public final class Conversations {
    ...
    public static ConversationChannelClient createClient(final Channel channel, final Executor executor) {
        return createClient(channel, OptionMap.EMPTY, executor);
    }
     ...
     * @return the conversation client
     */
    public static ConversationChannelClient createClient(final Channel channel, final OptionMap options, final Executor executor) {
        final ConversationChannelClientImpl client = new ConversationChannelClientImpl(channel, options, executor);
     ...
     */
    public static ConversationChannelHandler createServerReceiver(final ConversationMessageReceiver receiver, final Executor executor) {
        return createServerReceiver(receiver, OptionMap.EMPTY, executor);
    }

    /**
     * Create a server channel receiver.
     *
     * @param receiver the the conversation message receiver
     * @param options  the options
     * @param executor the executor
     * @return the server receiver
     */
    public static ConversationChannelHandler createServerReceiver(final ConversationMessageReceiver receiver, final OptionMap options, final Executor executor) {
        return new ConversationsServerHandler(receiver, options, executor);
    }
    ...
    public abstract static class AbstractMessageHandler implements Conversation.MessageHandler {
        ...
        public void handleMessage(Conversation conversation, InputStream message) throws IOException {
            try {
                // Handle the message and make sure that the input stream gets closed
